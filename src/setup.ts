...
import { ensureNZBHome, ENV_PATH, NZB_HOME } from "./paths.js";
...
async function askPicker(
	...
	const defaultIdx = Math.max(
		0,
		options.findIndex((o) => o.id === defaultId),
	);
...
`);
	...
	console.log(`${DIM}Config directory: ${NZB_HOME}${RESET}\n`);
	...
	// ── What is NZB ──────────────────────────────────────────
	console.log(`${BOLD}Meet NZB${RESET}`);
	console.log(`NZB is your personal AI assistant — an always-on daemon that runs on`);
	console.log(`your machine. Talk to him in plain English and he'll handle the rest.`);
	console.log();
	console.log(`${CYAN}What NZB can do out of the box:${RESET}`);
	...
	console.log();
	console.log(`${CYAN}Skills — teach NZB anything:${RESET}`);
	console.log(`  NZB has a skill system that lets him learn new capabilities. There's`);
	...
	console.log();
	console.log(`  ${DIM}"Check my email"${RESET}        → NZB researches how, writes a skill, does it`);
	console.log(`  ${DIM}"Turn off the lights"${RESET}   → NZB finds the right CLI tool, learns it`);
	console.log(`  ${DIM}"Find me a skill for"${RESET}   → NZB searches community skills and installs one`);
	console.log(`  ${DIM}"Learn how to use X"${RESET}    → NZB proactively learns before you need it`);
	console.log();
	console.log(`  Skills are saved permanently — NZB only needs to learn once.`);
	console.log();
	console.log(`${CYAN}How to talk to NZB:${RESET}`);
	console.log(`  • ${BOLD}Terminal${RESET}  — ${CYAN}nzb tui${RESET} — always available, no setup needed`);
	console.log(`  • ${BOLD}Telegram${RESET} — control NZB from your phone (optional, set up next)`);
	console.log();
	...
	console.log(`Telegram lets you talk to NZB from your phone — send messages,`);
	...
	if (setupTelegram) {
		...
		console.log(`  NZB uses your Telegram user ID to ensure only YOU can control it.`);
		...
		console.log(`\n${GREEN}  Done. Telegram locked down — only user ${userId} can control NZB.${RESET}`);
		...
		console.log(`\n${DIM}  Skipping Telegram. You can always set it up later with: nzb setup${RESET}\n`);
	}

	// ── Google (gogcli) Setup ─────────────────────────────────
	console.log(`${BOLD}━━━ Google / Gmail Setup (optional) ━━━${RESET}\n`);
	console.log(`NZB includes a Google skill that lets him read your email, manage`);
	...
	if (setupGoogle) {
		...
		console.log();
		console.log(`  This opens a browser for OAuth authorization. Once done, NZB can`);
		...
		console.log(`\n${GREEN}  Done. Google skill is ready — authenticate with gog auth add when you're set.${RESET}\n`);
	} else {
	...
	console.log(`${DIM}You can switch models anytime by telling NZB "switch to gpt-4.1"${RESET}\n`);
...
${BOLD}Get started:${RESET}
  ...
  ${CYAN}2.${RESET} Start NZB:
     ${BOLD}nzb start${RESET}

  ${CYAN}3.${RESET} ${setupTelegram ? "Open Telegram and message your bot!" : "Connect via terminal:"}
     ${BOLD}${setupTelegram ? "(message your bot on Telegram)" : "nzb tui"}${RESET}
